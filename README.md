![build](https://github.com/clc-assistant/github-action/workflows/build/badge.svg)

# Handling CLCs and DCOs via GitHub Action

Streamline your workflow and let this GitHub Action (a lite version of [CLC Assistant](https://github.com/clc-assistant/clc-assistant)) handle the legal side of contributions to a repository for you. CLC assistant GitHub action enables contributors to sign CLCs from within a pull request. With this GitHub Action we could get rid of the need for a centrally managed database by **storing the contributor's signature data** in a decentralized way - **in the same repository's file system** or **in a remote repository** which can be even a private repository.
...
1. contributors can sign the CLC or DCO by just posting a Pull Request comment
...
#### 1. Add the following Workflow File to your repository in this path`.github/workflows/clc.yml`
  ...
  CLCAssistant:
    ...
    steps:
      - name: 'CLC Assistant'
        ...
        with:
          path-to-signatures: 'signatures/version1/clc.json'
          path-to-document: 'https://github.com/clc-assistant/github-action/blob/master/SAPCLC.md' # e.g. a CLC or a DCO document
          ...
          #create-file-commit-message: 'For example: Creating file for storing CLC Signatures'
          #signed-commit-message: 'For example: $contributorName has signed the CLC in $owner/$repo#$pullRequestNo'
          #custom-notsigned-prcomment: 'pull request comment with Introductory message to ask new contributors to sign'
          #custom-pr-sign-comment: 'The signature to be committed in order to sign the CLC'
          #custom-allsigned-prcomment: 'pull request comment when all contributors has signed, defaults to **CLC Assistant Lite bot** All Contributors have signed the CLC.'
          #lock-pullrequest-aftermerge: false - if you don't want this bot to automatically lock the pull request after merging (default - true)
          #use-dco-flag: true - If you are using DCO instead of CLC
```

##### Demo for step 1

![add-clc-file](https://github.com/clc-assistant/github-action/blob/master/images/adding-clcfile.gif?raw=true)

#### 2. Pull Request event triggers CLC Workflow

CLC action workflow will be triggered on all Pull Request `opened, synchronize, closed`. This workflow will always run in the base repository and that's why we are making use of the [pull_request_target](https://docs.github.com/en/actions/reference/events-that-trigger-workflows#pull_request_target) event.
<br/> When the CLC workflow is triggered on pull request `closed` event, it will lock the Pull Request conversation after the Pull Request merge so that the contributors cannot modify or delete the signatures (Pull Request comment) later. This feature is optional.

#### 3. Signing the CLC

CLC workflow creates a comment on Pull Request asking contributors who have not signed CLC to sign and also fails the pull request status check with a `failure`. The contributors are requested to sign the CLC within the pull request by copy and pasting **"I have read the CLC Document and I hereby sign the CLC"** as a Pull Request comment like below.
If the contributor has already signed the CLC, then the PR status will pass with `success`. <br/>

##### Demo for step 2 and 3

![signature-process](https://github.com/clc-assistant/github-action/blob/master/images/signature-process.gif?raw=true)

<br/>

#### 4. Signatures stored in a JSON file

After the contributor signed a CLC, the contributor's signature with metadata will be stored in a JSON file inside the repository and you can specify the custom path to this file with `path-to-signatures` input in the workflow. <br/> The default path is `path-to-signatures: 'signatures/version1/clc.json'`.
...
and `remote-repository-name`: `<your repo name>` in your CLC workflow file.
...
![signature-storage-file](https://github.com/clc-assistant/github-action/blob/master/images/signature-storage-file.gif?raw=true)

#### 5. Users and bots in allowlist

If a GitHub username is included in the allowlist, they will not be required to sign a CLC. You can make use of this feature If you don't want your colleagues working in the same team/organisation to sign a CLC. And also, since there's no way for bot users (such as Dependabot or Greenkeeper) to sign a CLC, you may want to add them in `allowlist`. You can do so by adding their names in a comma separated string to the `allowlist` input in the CLC workflow file(in this case `dependabot[bot],greenkeeper[bot]`). You can also use wildcard symbol in case you want to allow all bot users something like `bot*`.

##### Demo for step 5

![allowlist](https://github.com/clc-assistant/github-action/blob/master/images/allowlist.gif?raw=true)
...
![personal-access-token](https://github.com/clc-assistant/github-action/blob/master/images/personal-access-token.gif?raw=true)

### Environmental Variables:

| Name                    | Requirement | Description                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GITHUB_TOKEN`          | _required_  | Usage: `GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}`, CLC Action uses this in-built GitHub token to make the API calls for interacting with GitHub. It is built into Github Actions and does not need to be manually specified in your secrets store. [More Info](https://help.github.com/en/actions/configuring-and-managing-workflows/authenticating-with-the-github_token)              |
...
| ----------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `path-to-document`            | _required_  | provide full URL `https://<clcfile>` to the document which shall be signed by the contributor(s) It can be any file e.g. inside the repository or it can be a gist.                                                                       | https://github.com/clc-assistant/github-action/blob/master/SAPCLC.md                   |
| `path-to-signatures`          | _optional_  | Path to the JSON file where all the signatures of the contributors will be stored inside the repository.                                                                                                                                  | signatures/version1/clc.json                                                           |
...
| `allowlist`                   | _optional_  | You can specify users and bots to be [added in allowlist](https://github.com/clc-assistant/github-action#5-users-and-bots-in-allowlist).                                                                                                  | user1,user2,bot\*                                                                      |
...
| `create-file-commit-message`  | _optional_  | Commit message when a new CLC file is created.                                                                                                                                                                                            | Creating file for storing CLC Signatures.                                              |
| `signed-commit-message`       | _optional_  | Commit message when a new contributor signs the CLC in a Pull Request.                                                                                                                                                                    | $contributorName has signed the CLC in $pullRequestNo                                  |
| `custom-notsigned-prcomment`  | _optional_  | Introductory Pull Request comment to ask new contributors to sign.                                                                                                                                                                        | Thank you for your contribution and please kindly read and sign our $pathToCLCDocument |
| `custom-pr-sign-comment`      | _optional_  | The signature to be committed in order to sign the CLC.                                                                                                                                                                                   | I have read the Developer Terms Document and I hereby accept the Terms                 |
| `custom-allsigned-prcomment`  | _optional_  | pull request comment when everyone has signed                                                                                                                                                                                             | All Contributors have signed the CLC.                                                  |
